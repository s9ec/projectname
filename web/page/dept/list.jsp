...
<div class="x-body">
    <div class="layui-row" style="" align="center">
        <form class="layui-form layui-col-md12 x-so" method="get" action="${ctx }/dept/search">
            <!-- <input class="layui-input" placeholder="开始日" name="start" id="start">
            <input class="layui-input" placeholder="截止日" name="end" id="end"> -->
            <input type="text" name="sxy"  value="" style="width:50%;"  placeholder="请输入查找设施名称" autocomplete="off" class="layui-input">
            <button class="layui-btn" style="background-color: #248cf5;" lay-submit="" lay-filter="sreach"><i class="layui-icon" style="background-color: #248cf5;">&#xe615;</i></button>
                  <select size='' name='method' multiple='multiple' style="float: right" class="layui-input">
                    <option selected='selected'  value="1">以设施编号查找</option>
    ...
    <table class="layui-table">
        ...
        <c:forEach items="${requestScope.M}" var="dept" varStatus="stat">
            <tr>
                ...
                <td>${dept.facility_id}</td>
                <td>${dept.facility_name}</td>
                <td>${dept.facility_business}</td>
                <td>${dept.phone}</td>
                <td>${dept.duty_people}</td>
                <td>${dept.control_people}</td>
                <td>${dept.money}</td>
                <td><a href="${ctx}/dept/check?facility_id=${dept.facility_id }">查看</a></td>
                <td class="td-manage">
                    <a title="编辑"  href="${ctx}/dept/update?facility_id=${dept.facility_id }">
                        <i class="layui-icon">&#xe642;</i>
                    </a>
                    <a title="删除" onclick="member_del(this,'${dept.facility_id }')" href="javascript:;">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
